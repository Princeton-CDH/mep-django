...
class WorldCatClientBase:
    ...
    def __init__(self):
        ...
        if not self.wskey:
            raise ImproperlyConfigured("OCLC search functionality " "requires a WSKEY.")
    ...
    def search(self, *args, **kwargs):
        ...
        try:
            response = self.session.get(
                ("%s/%s" % (self.WORLDCAT_API_BASE, self.API_ENDPOINT)).strip("/"),
                params=params,
            )
...
class WorldCatEntity:
    ...
    def item_type(self):
        ...
        rdf_types = list(
            str(rdf_type)
            for rdf_type in self.rdf_resource.objects(rdflib.RDF.type)
    ...
    def genres(self):
        ...
        return [str(genre) for genre in self.rdf_resource.objects(SCHEMA_ORG.genre)]
    ...
    def subjects(self):
        ...
        return [
            str(subject)
            for subject in self.rdf_resource.objects(SCHEMA_ORG.about)
            if "experiment.worldcat.org" not in str(subject)
            and "dewey.info" not in str(subject)
        ]
    ...
    def _lookup_to_search(*args, **kwargs):
        ...
        for key, value in kwargs.items():
            ...
            if isinstance(value, str):
                value = '"%s"' % value
            search_query.append(
                "%s%s%s%s" % (boolean_combination, field, operator, value)
            )
