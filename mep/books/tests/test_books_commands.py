...
class TestReconcileOCLC(TestCase):
    ...
    def test_command_line(self, mock_oclc_info, mockprogressbar):
        ...
        stdout = StringIO()
        call_command("reconcile_oclc", "report", "-o", csvtempfile.name, stdout=stdout)
        ...
        Work.objects.create(notes="PROBLEM some problematic issue here", slug="problem")
        ...
        work1 = Work.objects.create(
            title="Patriotic Adventurer", year=1936, slug="patriotic"
        )
        ...
        work2 = Work.objects.create(
            title="Crowded House", notes="Variant title", slug="crowded"
        )
        stdout = StringIO()
        ...
        with patch(
            "mep.books.management.commands.reconcile_oclc.Command.oclc_search_record"
        ) as mock_oclc_search:
            stdout = StringIO()
    ...
    def test_report(self):
        with patch.object(self.cmd, "oclc_info") as mock_oclc_info:
            ...
            work1 = Work.objects.create(
                title="Patriotic Adventurer", year=1936, slug="patriotic-adv"
            )
            # with one author to sanity-check included in output
            ctype = CreatorType.objects.get(name="Author")
            ...
            Creator.objects.create(creator_type=ctype, person=person, work=work1)
            work2 = Work.objects.create(
                title="Crowded House", notes="Variant title", slug="crowd"
            )
    ...
    def test_update_works(self):
        work1 = Work.objects.create(
            title="Time + Tide", notes="some notes", slug="time-tide"
        )
        with patch.object(self.cmd, "oclc_search_record") as mock_oclc_search:
            ...
            log_entry = LogEntry.objects.get(object_id=work.pk)
            assert log_entry.action_flag == CHANGE
            assert log_entry.change_message == "No OCLC match found"
    ...
    def test_oclc_search(self):
        ...
        work = Work.objects.create(
            title="Patriotic Adventurer", year=1936, slug="patriotic-adventurer"
        )
        # with one author to sanity-check included in output
        ctype = CreatorType.objects.get(name="Author")
        ...
        mock_sru_search.search.assert_called_with(
            title__exact=work.title, material_type__exact="book", **default_filters
        )
        ...
        with patch.object(
            Work, "first_known_interaction", new=datetime.date(1940, 1, 1)
        ):
            oclc_info = self.cmd.oclc_search(work)
...
class TestExportBooks(TestCase):
    ...
    def test_command_line(self):
        ...
        with patch(
            "mep.books.management.commands.export_books" + ".Command.get_object_data"
        ) as mock_get_obj_data:
            mock_get_obj_data.return_value = {"title": "test"}
            call_command("export_books", "-d", tempdir.name, "-m", 2, stdout=stdout)
