...
class TestWork(TestCase):
    ...
    def test_authors_editors_translators(self):
        ...
        Creator.objects.create(creator_type=editor_type, person=editor, work=work)
        Creator.objects.create(
    ...
    def test_populate_from_worldcat(self, mock_create_from_uri):
        ...
        def make_test_subject(uri):
            ...
            return Subject.objects.create(
                uri=uri, name="Lorton, Virginia", rdf_type="http://schema.org/Place"
            )
    ...
    def test_subject_list(self):
        ...
        subj1 = Subject.objects.create(
            uri="http://viaf.org/viaf/97006051",
            name="Ernest Hemingway",
            rdf_type="http://schema.org/Person",
        )
    ...
    def test_sort_author_list(self):
        ...
        Creator.objects.create(creator_type=author_type, person=author1, work=work)
        assert work.sort_author_list == "Smith, Bob"
        # multiple authors
        author2 = Person.objects.create(
            name="Bill Jones", sort_name="Jones, Bill", slug="j"
        )
        Creator.objects.create(creator_type=author_type, person=author2, work=work)
    ...
    def test_index_data(self):
        ...
        person = Person.objects.create(
            name="Joyce", slug="joyce", sort_name="Joyce, J."
        )
        translator_type = CreatorType.objects.get(name="Translator")
        translator = Person.objects.create(name="Juan Smythe", slug="sm")
        Creator.objects.create(creator_type=ctype, person=person, work=work1)
        Creator.objects.create(
            creator_type=translator_type, person=translator, work=work1
        )
        data = work1.index_data()
        ...
        Event.objects.create(
            account=acct, work=work1, start_date=datetime.date(1919, 11, 15)
        )
        data = work1.index_data()
    ...
    def test_items_to_index(self):
        ...
        with patch.object(Work, "objects") as mockobjects:
            ...
            mockobjects.prefetch_related.return_value.count_events.assert_any_call()
...
class TestSubject(TestCase):
    def get_test_subject(self):
        return Subject(
            name="Mark Twain",
            uri="https://viaf.org/viaf/50566653",
            rdf_type="http://schema.org/Person",
        )
    ...
    def test_create_from_uri(self, mock_requests):
        ...
        with open(os.path.join(FIXTURE_DIR, "loc_sh2008113651.jsonld")) as jsonld_file:
            ...
            mock_requests.get.assert_called_with("%s.jsonld" % loc_uri, headers={})
...
class TestEdition(TestCase):
    def test_str(self):
        work = Work.objects.create(title="Le foo et le bar", year=1916, slug="foo-bar")
        ...
        edition.volume = 2
        assert str(edition) == "%s (%s) vol. 2" % (edition.title, edition.partial_date)
        edition.number = 3
    ...
    def test_repr(self):
        work = Work.objects.create(
            title="Le foo et le bar", year=1916, slug="foo-bar-1916"
        )
    ...
    def test_display_html(self):
        ...
        assert edition.display_html() == "no. 19a, Winter 1931 <br/><em>Subsection</em>"

        # title and year but no season
        edition.season = ""
        assert edition.display_html() == "no. 19a, 1931 <br/><em>Subsection</em>"

    def test_display_text(self):
        work = Work.objects.create(title="transition", slug="trans")
        # volume only
        edition = Edition(
            work=work, volume=3, date=datetime.date(1931, 1, 1), title="subtitle"
        )
        assert edition.display_text() == "Vol. 3, 1931 subtitle"
...
class TestEditionCreator(TestCase):
    def test_str(self):
        ...
        creator = EditionCreator(creator_type=ctype, person=person, edition=edition)
