...
def formfield_selected_filter(context, boundfield):
    ...
    if isinstance(boundfield.field, forms.BooleanField):
        ...
        for val in value:
            ...
            # ** don't blow up on invalid choice
            index = [
                i
                for (i, choice) in enumerate(boundfield.field.choices)
                if choice[0] == val
            ]
            if index:  # if we found a matching index, use the first one
                index = index[0]
                input_id = boundfield.subwidgets[index].id_for_label
                links.append(
                    '<a data-input="%s" href="?%s">%s</a>'
                    % (
                        input_id,
...
def partialdate(val, date_format=None):
    ...
    if precision is not None:
        ...
        date_format = date_format.replace(" , ", " ").strip(
            "".join([string.punctuation, string.whitespace])
        )
