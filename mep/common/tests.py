...
class TestAdminDashboard(TestCase):
    def test_dashboard(self):
        ...
        assert re.search(
            "Library Accounts.*Personography",
            response_content,
            flags=re.MULTILINE | re.DOTALL,
        )
        # personography before bibliography
        assert re.search(
            "Personography.*Bibliography",
            response_content,
            flags=re.MULTILINE | re.DOTALL,
        )
        # bibliography before footnotes
        assert re.search(
            "Bibliography.*Footnotes", response_content, flags=re.MULTILINE | re.DOTALL
        )
        # bibliography before footnotes
        assert re.search(
            "Footnotes.*Administration",
            response_content,
            flags=re.MULTILINE | re.DOTALL,
        )
        ...
        for admin_url in [
            "people_person_changelist",
            "accounts_account_changelist",
            "accounts_borrow_changelist",
            "footnotes_footnote_changelist",
...
def test_absolutize_url():
    ...
    with override_settings(DEBUG=True):
        ...
        assert absolutize_url(local_path, mockrqst) == "http://example.org/sub/foo/bar/"
...
def test_alpha_pagelabels():
    ...
    labels = alpha_pagelabels(
        paginator, items, lambda x: getattr(x, "title"), max_chars=3
    )
    assert labels[1] == "Abi – Abn"
...
def test_iiif_image():
    ...
    assert str(mep_tags.iiif_image(myimg, "size:width=250")) == str(
        myimg.size(width=250)
    )
    assert str(mep_tags.iiif_image(myimg, "size:width=250,height=300")) == str(
...
def test_formfield_selected_filter():
    ...
    form.set_choices_from_facets({"gender": OrderedDict([("Female", 0), ("Male", 0)])})
    ...
    querystring = QueryDict(
        "has_card=1&page=2&sort=relevance&query=stein"
        + "&membership_dates_0=1920&membership_dates_1=test"
    )
    context = {"request": Mock(GET=querystring)}
    link = mep_tags.formfield_selected_filter(context, form["membership_dates"])
    ...
    link = mep_tags.formfield_selected_filter(context, form["birth_year"])
    ...
    context = {"request": Mock(GET=querystring)}
    link = mep_tags.formfield_selected_filter(context, form["gender"])
    ...
    form = MemberSearchForm(data={"nationality": "foobar"})
        ...
        checkbox_fieldset.optgroups.return_value = [
            (
                ...
                [
                    ...
                    {"label": "B", "attrs": {"id": "id_for_1"}, "value": "b"},
                ],
                0,
            )
        ]
...
class TestVaryOnHeadersMixin(TestCase):
    def test_vary_on_headers_mixing(self):
        ...
        vary_on_view = views.VaryOnHeadersMixin(vary_headers=["X-Foobar", "X-Bazbar"])
...
class TestFacetJSONMixin(TestCase):
    def test_render_response(self):
        ...
        view.object_list.get_facets.return_value = {"facets": "foo"}
...
class TestRdfViewMixin(TestCase):
    ...
    def test_rdf_graph(self):
        ...
        assert (page_node, rdflib.RDF.type, SCHEMA_ORG.WebPage) in graph
        ...
        home_crumb = graph.value(
            predicate=SCHEMA_ORG.item, object=rdflib.Literal("/"), any=False
        )
        page_crumb = graph.value(
            predicate=SCHEMA_ORG.item, object=rdflib.Literal("/my-page"), any=False
        )
...
class TestBreadcrumbsTemplate(TestCase):
    ...
    def test_home(self):
        ...
        response = self.template.render(
            context={"breadcrumbs": [("My Page", "/my-page")]}
        )
        assert '<li class="home">' not in response
        # with a 'home' crumb
        response = self.template.render(context={"breadcrumbs": [("Home", "/")]})
        assert '<li class="home">' in response

    def test_last_crumb(self):
        response = self.template.render(
            context={"breadcrumbs": [("Home", "/"), ("My Page", "/my-page")]}
        )
...
class TestBaseExport(TestCase):
    def test_flatten_dict(self):
        ...
        # nested with list
        nested_list = {"page": {"id": "p1", "label": ["one", "two"]}}
