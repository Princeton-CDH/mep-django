...
class MembersList(
    ...
    page_title = "Members"
    page_description = (
        "Search and browse members by name and filter "
    ...
    solr_sort = {"relevance": "-score", "name": "sort_name_isort"}
    ...
    def get_queryset(self):
        sqs = (
            ...
            .facet_field("arrondissement", exclude="arrondissement", sort="value")
        )
        ...
        # when form is valid, check for search term and filter queryset
            ...
            if search_opts["query"]:
                sqs = (
                    sqs.search(self.search_name_query)
            ...
            if search_opts["nationality"]:
                ...
                nationality_list = [
                    '"%s"' % val if " " in val else val
                    for val in search_opts["nationality"]
                ]
                sqs = sqs.filter(nationality__in=nationality_list, tag="nationality")
    ...
    def get_page_labels(self, paginator):
        ...
        pagination_qs = self.queryset.only("sort_name").get_results(rows=100000)
        alpha_labels = alpha_pagelabels(
            paginator, pagination_qs, lambda x: x["sort_name"][0], max_chars=4
        )
...
class MemberPastSlugMixin:
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            person = (
                Person.objects.library_members()
                .filter(past_slugs__slug=self.kwargs["slug"])
                .first()
            )
...
class MemberDetail(
    MemberPastSlugMixin, MemberLastModifiedListMixin, DetailView, RdfViewMixin
):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["timeline"] = {
            "membership_activities": [
                ...
                for event in account.event_set.membership_activities().known_years()
            ],
            "book_activities": [
                {"startDate": start_date, "count": count}
        ...
        # members search map
        addresses = (
            Address.objects.filter(account=account)
        ...
        membership_years = strip_tags(
            as_ranges(account_years).replace("</span>", ",")
        ).rstrip(",")
...
class MembershipActivities(
    MemberPastSlugMixin, MemberLastModifiedListMixin, ListView, RdfViewMixin
):
    ...
    def get_context_data(self, **kwargs):
        ...
        self.member = get_object_or_404(
            Person.objects.library_members(), slug=self.kwargs["slug"]
        )
        context = super().get_context_data(**kwargs)
        context.update(
            {
    ...
    def get_absolute_url(self):
        ...
        return absolutize_url(
            reverse("people:membership-activities", kwargs=self.kwargs)
        )
...
class BorrowingActivities(
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .prefetch_related(
                "work__creators", "work__creator_set", "work__creator_set__creator_type"
            )
        )
    ...
    def get_absolute_url(self):
        """Get the full URI of this page."""
        return absolutize_url(
            reverse("people:borrowing-activities", kwargs=self.kwargs)
        )
...
class MemberCardList(
    ...
    def get_absolute_url(self):
        ...
        return absolutize_url(reverse("people:member-card-list", kwargs=self.kwargs))
...
class MemberCardDetail(
    ...
    def get_absolute_url(self):
        """Full URI for member card list page."""
        return absolutize_url(reverse("people:member-card-detail", kwargs=self.kwargs))
    ...
    def get_breadcrumbs(self):
        ...
        return [
            ...
            (
                ...
                absolutize_url(
                    reverse(
                        "people:member-card-list", kwargs={"slug": self.member.slug}
                    )
                ),
            ),
    ...
    def get_context_data(self, **kwargs):
        ...
        member_events = self.object.footnote_set.events().filter(
            account__persons=self.member
        )
...
class PersonAutocomplete(autocomplete.Select2QuerySetView):
    ...
    def get_result_label(self, person):
        ...
        labels["main_string"] = ("%s %s" % (person.title, person.name)).strip()
        # format birth-death in a familiar pattern ( - )
        if person.birth_year or person.death_year:
            labels["bio_dates"] = " (%s – %s)" % (person.birth_year, person.death_year)
        ...
        if not labels["bio_dates"] and not labels["note_string"]:
            ...
            if person.account_set.first():
                event = (
                    Event.objects.filter(account=person.account_set.first())
                    .order_by("start_date")
                    .first()
                )
                ...
                if event:
                    ...
                    labels["end_date"] = event.end_date if event.end_date else ""
    ...
    def get_queryset(self):
        ...
        return Person.objects.filter(
            Q(name__icontains=self.q) | Q(mep_id__icontains=self.q)
        )
...
class PersonMerge(PermissionRequiredMixin, FormView):
    ...
    def get_success_url(self):
        ...
        # otherwise
        querystring = (
            "?%s" % self.request.session.get("people_merge_filter", "")
        ).rstrip("?")
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            Person.objects.filter(id__in=self.person_ids).merge_with(primary_person)
            ...
            if primary_person.has_account():  # calculate events reassociated
                primary_account = (
                    primary_person.account_set.first()
