...
class TestPeopleViews(TestCase):
    ...
    def test_geonames_autocomplete(self, mockgeonamesapi):
        ...
        mockgeonamesapi.return_value.search.assert_called_with(
            "new york", max_rows=50, name_start=True
        )
        ...
        assert work["id"] == GeoNamesAPI.uri_from_id(mock_response[0]["geonameId"])
    ...
    def test_person_autocomplete(self):
        ...
        beach = Person.objects.create(
            name="Sylvia Beach", mep_id="sylv.b", slug="beach"
        )
        ...
        ms_sylvia = Person.objects.create(name="Sylvia", title="Ms.", slug="sylvia")
        ...
        # on a separate line
        expected = (
            "<strong>Sylvia Beach (1900 – 1970)</strong> "
        ...
        text = data["results"][0]["text"]
        expected = (
            "<strong>Ms. Sylvia</strong> sylv.a <br />"
    ...
    def test_person_admin_change(self):
        ...
        superuser = User.objects.create_superuser(
            username="admin", password=su_password, email="su@example.com"
        )
        ...
        mlle_dufour = Person.objects.create(name="Dufour", title="Mlle", slug="dufour")
        parent = RelationshipType.objects.create(name="parent")
        rel = Relationship.objects.create(
            from_person=mlle_dufour,
            relationship_type=parent,
            to_person=m_dufour,
            notes="relationship uncertain",
        )
        person_edit_url = reverse("admin:people_person_change", args=[m_dufour.id])
        ...
        self.assertContains(
            result,
            reverse("admin:people_person_change", args=[mlle_dufour.id]),
            msg_prefix="should include edit link for person related to this person",
        )
        self.assertContains(
            result, parent.name, msg_prefix="should include relationship name"
        )
        self.assertContains(
            result, rel.notes, msg_prefix="should include any relationship notes"
        )
        ...
        subs = Subscription.objects.create(
            account=acct, start_date=date(1943, 1, 1), end_date=date(1944, 1, 1)
        )
        subs2 = Subscription.objects.create(
            account=acct, start_date=date(1944, 1, 1), subtype=Subscription.RENEWAL
        )
        reimb = Reimbursement.objects.create(account=acct, start_date=date(1944, 2, 1))
        ...
        self.assertContains(
            response, "%s - %s" % (subs.partial_start_date, subs.partial_end_date)
        )
    ...
    def test_person_admin_list(self):
        ...
        self.assertContains(
            result,
            Person.address_count.short_description,
            msg_prefix="should have address_count field and short_desc.",
        )
        self.assertContains(
            result,
            Person.list_nationalities.short_description,
    ...
    def test_person_merge(self):
        ...
        perms = Permission.objects.filter(
            codename__in=["change_person", "delete_person"]
        )
        ...
        Creator.objects.create(
            creator_type=author, person=pers, work=book1
        )  # pers author of book1
        Creator.objects.create(
            creator_type=editor, person=pers2, work=book2
        )  # pers2 editor of book2
        Creator.objects.create(
            creator_type=author, person=pers2, work=book2
        )  # pers2 author of book2
        ...
        response = self.client.post(
            "%s?ids=%s" % (reverse("people:merge"), idstring),
        ...
        assert reverse("admin:people_person_change", args=[pers.id]) in message.message
        assert (
            reverse("admin:accounts_account_change", args=[acct.id]) in message.message
        )
        ...
        assert reverse("admin:people_person_change", args=[pers3.id]) in message.message
...
class TestGeonamesLookupWidget(TestCase):
    def test_render(self):
        ...
        assert '<p><a id="geonames_uri" target="_blank" href=""></a></p>' in rendered
        ...
        assert (
            '<a id="geonames_uri" target="_blank" href="%(uri)s">%(uri)s</a>'
            % {"uri": uri}
            in rendered
        )
        ...
        assert (
            '<option value="%(uri)s" selected>%(uri)s</option' % {"uri": uri}
            in rendered
        )
...
class TestMembersListView(TestCase):
    ...
    def test_list(self):
        ...
        card = Bibliography.objects.create(
            bibliographic_note="A Library Card", source_type=src_type
        )
        account.card = card
        account.save()
        ...
        # gender facet & nationality both have an info link
        self.assertContains(response, 'class="info-link"', count=2)
    ...
    def test_get_form(self, mocksuper):
        ...
        view.get_year_ranges.return_value = {"birth_year": (1900, 1920)}
        view.get_form()
        # cached form is set
    ...
    def test_get_queryset(self, mock_solrqueryset):
        ...
        view.request = self.factory.get(
            self.members_url, {"has_card": True, "query": "", "gender": ["Female", ""]}
        )
        # remove cached form
        del view._form
        sqs = view.get_queryset()
        ...
        view.request = self.factory.get(
            self.members_url, {"membership_dates_0": 1919, "membership_dates_1": 1923}
        )
        del view._form
        sqs = view.get_queryset()
        ...
        view.request = self.factory.get(
            self.members_url, {"query": "", "nationality": ["France"]}
        )
        del view._form
        sqs = view.get_queryset()
        mock_qs.filter.assert_any_call(nationality__in=["France"], tag="nationality")

        # filter on arrondissement
        view.request = self.factory.get(
            self.members_url, {"query": "", "arrondissement": ["6th"]}
        )
        del view._form
        sqs = view.get_queryset()
        mock_qs.filter.assert_any_call(arrondissement__in=["6"], tag="arrondissement")
    ...
    def test_get_range_stats(self, mockPSQ):
        ...
        mock_stats = {
            "stats_fields": {
                "account_years": {"min": 1928.0, "max": 1940.0},
                "birth_year": {"min": 1910.0, "max": 1932.0},
            }
        }
        mockPSQ.return_value.stats.return_value.get_stats.return_value = mock_stats
...
class TestMemberDetailView(TestCase):
    ...
    def test_get_member(self):
        ...
        assert response.status_code == 200, "library members should have a detail page"
        assert (
            response.context["member"] == gay
    ...
    def test_last_modified(self, mock_wsq):
        ...
        mock_wsq.return_value.filter.assert_called_with(
            item_type="person", slug_s=gay.slug
        )
    ...
    def test_get_non_member(self):
        ...
        response = self.client.get(url)
        assert response.status_code == 404, "non-members should not have a detail page"
...
class TestPastSlugRedirects(TestCase):
    ...
    def setUp(self):
        ...
        PastPersonSlug.objects.create(person=self.member, slug=self.old_slug)
        aeschylus = Person.objects.get(name="Aeschylus", slug="aeschylus")
        self.nonmember_slug = "aeschy"
        PastPersonSlug.objects.create(person=aeschylus, slug=self.nonmember_slug)
    ...
    def test_member_card_detail_page(self):
        ...
        response = self.client.get(
            reverse(route, kwargs={"slug": self.old_slug, "short_id": self.canvas_id})
        )

        assert response.status_code == 301  # permanent redirect
        # redirect to same view with the *correct* slug
        assert response["location"].endswith(
            reverse(route, kwargs={"slug": self.slug, "short_id": self.canvas_id})
        )

        # non-member old slug should still 404
        response = self.client.get(
            reverse(
                route, kwargs={"slug": self.nonmember_slug, "short_id": self.canvas_id}
            )
        )
        assert response.status_code == 404
...
class TestMembershipActivities(TestCase):
    ...
    def setUp(self):
        ...
        self.events = {
            "subscription": Subscription.objects.create(
                account=acct,
                category=subtype,
                start_date=date(1920, 3, 1),
                end_date=date(1920, 4, 1),
                price_paid=30,
                currency="FRF",
            ),
            "reimbursement": Reimbursement.objects.create(
                account=acct,
                start_date=date(1920, 5, 5),
                end_date=date(1920, 5, 5),
                refund=15,
                currency="USD",
            ),
    ...
    def test_get_absolute_url(self):
        assert self.view.get_absolute_url() == absolutize_url(
            reverse("people:membership-activities", kwargs={"slug": self.member.slug})
        )
    ...
    def test_view_template(self):
        ...
        self.assertContains(response, 'data-sort="%s"' % subs.partial_start_date)
        self.assertContains(response, 'data-sort="%s"' % subs.partial_end_date)
        ...
        self.assertContains(
            response, 'data-sort="%s"' % reimburse.partial_start_date, count=2
        )
        ...
        response = self.client.get(
            reverse("people:membership-activities", kwargs={"slug": "gay"})
        )
...
class TestBorrowingActivities(TestCase):
    ...
    def test_get_absolute_url(self):
        assert self.view.get_absolute_url() == absolutize_url(
            reverse("people:borrowing-activities", kwargs={"slug": self.member.slug})
        )
    ...
    def test_view_template(self):
        ...
        response = self.client.get(
            reverse("people:borrowing-activities", kwargs={"slug": "gay"})
        )
        self.assertNotContains(response, "<table")
...
class TestMemberCardList(TestCase):
    ...
    def setUp(self):
        self.factory = RequestFactory()
        self.cardlist_url = reverse(
            "people:member-card-list", kwargs={"slug": "stein-gertrude"}
        )
    ...
    def test_get_absolute_url(self):
        ...
        assert self.view.get_absolute_url() == absolutize_url(
            reverse("people:member-card-list", kwargs=self.view.kwargs)
        )
    ...
    def test_get_breadcrumbs(self):
        ...
        assert crumbs[-2][1] == absolutize_url(self.view.member.get_absolute_url())
    ...
    def test_view_template(self):
        ...
        response = self.client.get(
            reverse("people:member-card-list", kwargs={"slug": "stein-gertrude"})
        )
        ...
        response = self.client.get(
            reverse("people:member-card-list", kwargs={"slug": "gay"})
        )
        ...
        response = self.client.get(
            reverse("people:member-card-list", kwargs={"slug": "hemingway"})
        )
...
class TestMemberCardDetail(TestCase):
    ...
    def setUp(self):
        self.factory = RequestFactory()
        self.cardlist_url = reverse("people:member-card-detail", kwargs=self.kwargs)
    ...
    def test_get_absolute_url(self):
        ...
        assert self.view.get_absolute_url() == absolutize_url(
            reverse("people:member-card-detail", kwargs=self.view.kwargs)
        )
    ...
    def test_get_breadcrumbs(self):
        ...
        assert crumbs[-2][1] == absolutize_url(
            reverse("people:member-card-list", kwargs={"slug": "stein-gertrude"})
        )
        # third to last is member page
        assert crumbs[-3][0] == self.view.member.short_name
        assert crumbs[-3][1] == absolutize_url(self.view.member.get_absolute_url())
    ...
    def test_get_context_data(self):
        ...
        assert set(context["events"].values_list("pk", flat=True)) == set(
            expected_events.values_list("pk", flat=True)
        )
        ...
        assert list(context["cards"]) == list(
            self.view.member.account_set.first().member_card_images()
        )

    def test_view_template(self):
        member = Person.objects.get(slug="stein-gertrude")
        response = self.client.get(
            reverse("people:member-card-detail", kwargs=self.kwargs)
        )
        ...
        for i, card in enumerate(context["cards"].all()):
            ...
            self.assertContains(
                ...
                reverse(
                    "people:member-card-detail",
                    kwargs={"slug": context["member"].slug, "short_id": card.short_id},
                ),
            )
            # each card link has an alt
            self.assertContains(response, 'alt="Gertrude Stein card %d"' % (i + 1))
