...
from mep.accounts.models import Account, Address, Borrow, Reimbursement, Subscription
...
class TestPerson(TestCase):
    ...
    def test_subscription_dates(self):
        ...
        sub1 = Subscription.objects.create(
            start_date=date(1950, 1, 5), end_date=date(1950, 1, 8), account=acc
        )
        sub2 = Subscription.objects.create(start_date=date(1950, 2, 10), account=acc)
        assert pers.subscription_dates() == "; ".join(
            s.date_range for s in [sub1, sub2]
        )
    ...
    def test_get_absolute_url(self):
        ...
        acct.persons.add(pers)
        # uses pk for now
        assert pers.get_absolute_url() == reverse(
            "people:member-detail", args=[pers.slug]
        )

    def test_items_to_index(self):
        with patch.object(Person.objects, "library_members") as mock_lib_members:
    ...
    def test_index_data(self):
        ...
        Borrow.objects.create(
            account=acct, work=book1, start_date=datetime.date(1921, 4, 10)
        )
        ...
        month_unknown.save()
        Reimbursement.objects.create(account=acct, start_date=datetime.date(1922, 1, 1))
        ...
        assert set(index_data["account_years_is"]) == set([1921, 1922, 1930])
        assert set(index_data["account_yearmonths_is"]) == set(
            ["192101", "192102", "192104", "192201"]
        )
        assert set(index_data["logbook_yearmonths_is"]) == set(
            ["192101", "192102", "192201"]
        )
        assert set(index_data["card_yearmonths_is"]) == set(["192104"])
...
class TestPersonQuerySet(TestCase):
    ...
    def test_merge_with(self):
        ...
        with pytest.raises(MultipleObjectsReturned) as excinfo:
            Person.objects.merge_with(main_person)
        assert "Can't merge with a person record that has multiple accounts" in str(
            excinfo.value
        )
        main_person.delete()
        ...
        full = Person.objects.create(
            ...
            birth_year=1901,
            death_year=1950,
            viaf_id="http://viaf.org/viaf/123456",
            gender="M",
            notes="some details",
            profession=prof,
        )
        ...
        with pytest.raises(MultipleObjectsReturned) as excinfo:
            Person.objects.merge_with(main)
        assert "Can't merge a person record with a shared account." in str(
            excinfo.value
        )
        ...
        spencer = Person.objects.create(
            name="Spencer Hadley", birth_year=1990, slug="hadl-2"
        )
        ...
        spencer_address = Address.objects.create(
            account=spencer_acct, location=location
        )
        ...
        nikitas_address = Address.objects.create(
            account=nikitas_acct, location=location
        )
        ...
        # events were merged
        assert spencer_event in mike.account_set.first().get_events(
            etype="subscription"
        )
        assert nikitas_event in mike.account_set.first().get_events(
            etype="subscription"
        )
...
class TestRelationshipM2M(TestCase):
    def setUp(self):
        ...
        Relationship.objects.create(
            from_person=self.foo, to_person=self.bar, relationship_type=self.parent
        )
        ...
        Relationship.objects.create(
            from_person=self.foo, to_person=self.baz, relationship_type=self.partner
        )
    ...
    def test_relationships(self):
        ...
        assert (
            Person.objects.get(from_relationships__relationship_type=self.parent)
            == self.foo
        )
        # reverse - should get Bar
        assert (
            Person.objects.get(to_relationships__relationship_type=self.parent)
            == self.bar
        )
        # foo is partner of bas - should get Baz
        assert (
            Person.objects.get(to_relationships__relationship_type=self.partner)
            == self.baz
        )
        ...
        assert len(self.foo.relations.filter(name__in=["Baz", "Bar"])) == 2
...
class TestAddress(TestCase):
    def test_str(self):
        ...
        address = Location(street_address="1 Rue Le Foo", city="Paris", name="La Hotel")
    ...
    def test_latlon_validate(self):
        ...
        with pytest.raises(ValidationError) as excinfo:
            ...
            address.clean_fields()
        assert "Lat/Lon must be between -180 and 180 degrees." in str(excinfo.value)
...
class TestInfoURL(TestCase):
    ...
    def test_repr(self):
        ...
        # saved
        info_url.save()
        assert repr(info_url) == "<InfoURL pk:%s %s>" % (info_url.pk, info_url.url)
