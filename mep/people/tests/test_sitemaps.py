...
def test_solr_timestamp_to_date():
    assert sitemaps.solr_timestamp_to_date("2020-05-12T15:46:20.341Z") == date(
        2020, 5, 12
    )
...
class TestMemberSitemap:
    ...
    def test_items(self, mock_psqs):
        ...
        mock_psqs.return_value.all.return_value.only.assert_called_with(
            "slug", "last_modified"
        )

    def test_location(self):
        assert sitemaps.MemberSitemap().location(
            {"slug": "gstein", "last_modified": "2020-05-12T15:46:20.341Z"}
...
class TestMemberCardListSitemap:
    ...
    def test_items(self, mocksuper):
        ...
        mocksuper.return_value.items.return_value.only.assert_called_with(
            "slug", "last_modified", "has_card"
        )
...
class TestMemberCardDetailSitemap(TestCase):
    ...
    @patch("mep.people.sitemaps.PersonSolrQuerySet")
    def test_items(self, mock_psqs):
        mock_psqs.return_value.filter.return_value.only.return_value.get_results.return_value = [
            {"slug": "stein-gertrude", "last_modified": "2020-05-12T15:46:20.341Z"}
        ]
        ...
        canvas_ids = Canvas.objects.filter(
            manifest__bibliography__account__isnull=False
        ...
        mock_psqs.return_value.filter.return_value.only.assert_called_with(
            "slug", "last_modified"
        )
    ...
    def test_location(self):
        ...
        assert sitemaps.MemberCardDetailSitemap().location(obj) == reverse(
            "people:member-card-detail", args=["gstein", obj["short_id"]]
        )
