...
class TestCardList(TestCase):
    ...
    def test_list(self):
        ...
        for card in cards:
            ...
            self.assertContains(
                response, card.manifest.thumbnail.image.size(width=225), count=2
            )
            # 2x image appears once
            self.assertContains(
                response, card.manifest.thumbnail.image.size(width=225 * 2), count=1
            )
    ...
    def test_get_page_labels(self):
        ...
        view.queryset = Mock()
        with patch("mep.footnotes.views.alpha_pagelabels") as mock_alpha_pglabels:
            works = range(101)
