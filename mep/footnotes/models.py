...
class BibliographySignalHandlers:
    ...
    def debug_log(name, count, mode="save"):
        ...
        logger.debug(
            "%s %s, reindexing %d related card%s",
            mode,
            name,
            count,
            "" if count == 1 else "s",
    ...
    def person_delete(sender, instance, **kwargs):
        ...
        card_ids = Bibliography.objects.filter(
            account__persons__pk=instance.pk
        ...
        if card_ids:
            ...
            BibliographySignalHandlers.debug_log("person", cards.count(), mode="delete")
            ModelIndexable.index_items(cards)
    ...
    def account_delete(sender, instance, **kwargs):
        ...
        lending card"""
        card_ids = Bibliography.objects.filter(account__pk=instance.pk).values_list(
            "id", flat=True
        )
        ...
        if card_ids:
            ...
            # find the items based on the list of ids to reindex
            cards = Bibliography.objects.filter(id__in=list(card_ids))
            BibliographySignalHandlers.debug_log(
                "account", cards.count(), mode="delete"
            )
            ModelIndexable.index_items(cards)
    ...
    def manifest_delete(sender, instance, **kwargs):
        ...
        lending card"""
        card_ids = Bibliography.objects.filter(manifest__pk=instance.pk).values_list(
            "id", flat=True
        )
        if card_ids:
            # delete the assocation so cards will index without the account
            ...
            cards = Bibliography.objects.filter(id__in=list(card_ids))
            BibliographySignalHandlers.debug_log(
                "manifest", cards.count(), mode="delete"
            )
            ModelIndexable.index_items(cards)
    ...
    def canvas_delete(sender, instance, **kwargs):
        ...
        cards = Bibliography.objects.filter(manifest__pk=instance.manifest.pk)
        if cards.exists():
            BibliographySignalHandlers.debug_log("canvas", cards.count(), mode="delete")
            ModelIndexable.index_items(cards)
    ...
    def event_delete(sender, instance, **kwargs):
        ...
        cards = Bibliography.objects.filter(account__pk=instance.account.pk)
        if cards.exists():
            BibliographySignalHandlers.debug_log("event", cards.count(), mode="delete")
            ModelIndexable.index_items(cards)
...
class Bibliography(Notable, ModelIndexable):
    ...
    def items_to_index(cls):
        ...
        return cls.objects.filter(account__isnull=False, manifest__isnull=False)
    ...
    def index_data(self):
        ...
        for account in self.account_set.all():
            ...
            account_years.update(set(date.year for date in account.event_dates))
        if names:
            index_data.update(
                {
                    "cardholder_t": names,
...
class FootnoteQuerySet(models.QuerySet):
    ...
    def events(self):
        ...
        event_ids = self.filter(content_type=event_content_type).values_list(
            "object_id", flat=True
        )
    ...
    def event_date_range(self):
        ...
        date_values = (
            ...
            .aggregate(first=models.Min("start_dates"), last=models.Max("end_dates"))
        )
...
class Footnote(Notable):
    ...
    content_type = models.ForeignKey(
        ...
        limit_choices_to=models.Q(
            app_label="people", model__in=["country", "person", "address", "profession"]
        )
        | models.Q(app_label="accounts", model__in=["account", "event"])
    ...
    is_agree = models.BooleanField(
        "Supports",
        help_text="True if the evidence "
