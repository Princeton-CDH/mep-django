...
def add_missing_footnotes(apps, schema_editor):
    ...
    event_content_type = ContentType.objects.get(model="event", app_label="accounts")
    ...
    addressbook_1936 = Bibliography.objects.filter(
        bibliographic_note__contains="Address Book 1919–1935",
        source_type__name="Address Book",
    ).first()
    addressbook_post1936 = Bibliography.objects.filter(
        bibliographic_note__contains="Address Book 1935–1937",
        source_type__name="Address Book",
    ).first()
    logbooks = Bibliography.objects.filter(
        bibliographic_note__contains="Logbooks 1919–1941", source_type__name="Logbook"
    ).first()
    ...
    events_with_footnotes = Footnote.objects.filter(
        content_type=event_content_type
    ).values_list("object_id")

    if addressbook_post1936:
        # find all events with tag P36ADD and no footnote
        events_post36add = Event.objects.filter(notes__contains="P36ADD").exclude(
            pk__in=events_with_footnotes
        )
    ...
    if addressbook_1936:
        ...
        events_36add = (
            Event.objects.filter(notes__contains="36ADD")
    ...
    if logbooks:
        ...
        logbooks_events = Event.objects.exclude(notes__contains="36ADD").exclude(
            pk__in=events_with_footnotes
        )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(add_missing_footnotes, migrations.RunPython.noop)
    ]
