...
from mep.accounts.partial_date import (
    DatePrecision,
    DatePrecisionField,
    PartialDate,
    PartialDateMixin,
...
class Account(models.Model, EventSetMixin):
    ...
    persons = models.ManyToManyField(
        Person, blank=True, verbose_name="Account holder(s)"
    )
    ...
    def __repr__(self):
        ...
        if self.pk and self.persons.count():
            names = " %s" % ";".join([str(person) for person in self.persons.all()])
    ...
    def list_persons(self):
        ...
        account.
        """
        return ", ".join(person.name for person in self.persons.all().order_by("name"))
    ...
    def validate_etype(etype):
        ...
        if etype not in [
            ...
            raise ValueError(
                "etype must be one of borrow, event, purchase,"
    ...
    def member_card_images(self):
        ...
        manifest_cards = self.card.manifest.canvases.all().annotate(
        ...
        event_cards = (
            Canvas.objects.exclude(manifest=self.card.manifest)
    ...
    def clean(self):
        ...
        if not self.account and not self.person:
            raise ValidationError(
                "Address must be associated with an account or person"
            )
        if self.account and self.person:
            raise ValidationError(
                "Address must only be associated with one of account or person"
            )
...
class EventQuerySet(models.QuerySet):
    ...
    def membership_activities(self):
        ...
        return self.filter(
            models.Q(subscription__isnull=False) | models.Q(reimbursement__isnull=False)
        )
    ...
    def known_years(self):
        ...
        return self.exclude(start_date_precision__knownyear=False).exclude(
            end_date_precision__knownyear=False
        )
...
class Event(Notable, PartialDateMixin):
    ...
    re_nonstandard_notation = re.compile("(%s)" % "|".join(nonstandard_notation.keys()))
...
class CurrencyMixin(models.Model):
    ...
    currency = models.CharField(
        max_length=3, blank=True, choices=CURRENCY_CHOICES, default=FRF
    )
...
class Subscription(Event, CurrencyMixin):
    ...
    purchase_date = models.DateField(
        blank=True, null=True, help_text="Date the subscription was purchased."
    )
    ...
    def validate_unique(self, *args, **kwargs):
        ...
        try:
            qs = Reimbursement.objects.filter(
                start_date=self.start_date, account=self.account, refund=self.refund
            )
        except ObjectDoesNotExist:
