...
def can_tweet(ev, day):
    ...
    if (
        ev.event_label in ["Subscription", "Renewal"]
        and ev.subscription.partial_purchase_date != day
    ):
        return False
    ...
    return any(
        [
            ...
            ev.event_label in tweetable_event_types and ev.partial_start_date == day,
        ]
    )
...
def tweet_content(ev, day):
    ...
    prolog = "#100YearsAgoToday on %s at Shakespeare and Company, " % day.strftime(
        Command.date_format
    )
    # handle shared accounts
    member_name = " and ".join(m.firstname_last for m in ev.account.persons.all())
    tweet_info = {"member": member_name}
    event_label = ev.event_label
    tweet_pattern = None

    if (
        event_label in ["Subscription", "Renewal"]
        ...
        tweet_info.update({"verb": verb, "duration": "", "volumes": ""})
        # include duration if known
        if ev.subscription.duration:
            tweet_info["duration"] = " for %s" % ev.subscription.readable_duration()
...
def card_url(member, ev):
    ...
    if footnote and footnote.image:
        url = reverse(
            ...
            kwargs={"slug": member.slug, "short_id": footnote.image.short_id},
        )
