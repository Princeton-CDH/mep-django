...
class Command(BaseExport):
    ...
    def get_object_data(self, obj):
        ...
        if event_type in ["Subscription", "Supplement", "Renewal", "Separate Payment"]:
        ...
        # reimbursement data
            ...
            data["reimbursement"] = {"refund": "%.2f" % obj.reimbursement.refund}
            currency = obj.reimbursement.currency

        # borrow data
        elif event_type == "Borrow":
            data["borrow"] = {"status": obj.borrow.get_item_status_display()}
        ...
        # purchase data
            ...
            data["purchase"] = {"price": "%.2f" % obj.purchase.price}
        ...
        if obj.footnotes.exists():
            data["source"] = [self.source_info(fn) for fn in obj.footnotes.all()]
        return data
    ...
    def item_info(self, event):
        ...
        if event.work:
            ...
            if event.work.authors:
                item_info["authors"] = [a.sort_name for a in event.work.authors]
            if event.work.year:
