...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        card_bibliographies = Bibliography.objects.filter(
            notes__contains=self.pudl_basepath
        )

        self.stdout.write(
            "Found %d bibliographies with pudl image paths"
        ...
        with open(kwargs["csv"]) as csvfile:
            ...
            for row in reader:
                ...
                if manifest_id and manifest_id != row["scanned_resource.id"]:
                    ...
                    card = self.migrate_card_bibliography(manifest_uri, image_paths)
                    # migrate footnotes
                    if card:
        ...
        pudl_bibliographies = Bibliography.objects.filter(
            notes__contains=self.pudl_basepath
        )
        pudl_footnotes = Footnote.objects.filter(location__contains=self.pudl_basepath)
    ...
    def migrate_card_bibliography(self, manifest_uri, image_paths):
        ...
        if card is None:
            self.stderr.write("Could not identify card for %s" % ",".join(image_paths))
            return
    ...
    def migrate_footnotes(self, card, canvas_map):
        ...
        for imgpath, canvas_id in canvas_map.items():
            ...
            footnotes = card.footnote_set.filter(location__contains=imgpath_basename)
            if footnotes.exists():
                # get the canvas object
                canvas = card.manifest.canvases.get(uri=canvas_id)
                # update all cards at once
                footnote_ids = [fnote.pk for fnote in footnotes]
                footnotes.update(location=canvas_id, image=canvas, bibliography=card)
    ...
    def clean_footnotes(self):
        ...
        for fnote in Footnote.objects.filter(
            location__contains="dolan", bibliography__notes__contains="du_bois"
        ):
        ...
        for fnote in Footnote.objects.filter(
            location__contains="pudl", location__regex=r"[a-z]000"
        ):
    ...
    def clean_orphaned_footnotes(self):
        ...
        orphans = Footnote.objects.filter(location__contains="pudl").exclude(
            bibliography__notes__contains=models.F("location")
        )
        ...
        for footnote in orphans:
            ...
            location_path = os.path.splitext(
                footnote.location.partition("/pudl0123/")[-1]
            )[0]
            if location_path in footnote.bibliography.notes:
                continue
            bib = Bibliography.objects.filter(notes__contains=location_path)
            if bib.count() == 1:
